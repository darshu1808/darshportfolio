...
export default function Contact({ id }: ContactProps) {
  ...
  return (
    <section id={id} className="relative w-full min-h-screen flex flex-col items-center justify-center z-10 px-6 py-40 overflow-hidden">
      {/* Background */}
          ...
          <div className="flex justify-center gap-6">
            {(content.contact?.socials || []).map((social: any, i: number) => {
              ...
              <a
                ...
                className="w-14 h-14 rounded-2xl bg-white/[0.03] border border-white/5 flex items-center justify-center hover:border-white/20 hover:scale-110 transition-all duration-300 group"
                title={social.label}
              >
                <IconComponent className="w-5 h-5 text-white/50 group-hover:text-white transition-colors" />
              </a>
            )})}
          </div>
        </motion.div>
